...
export class BodycarePage {
  selectedSortOption: string = 'recentlyAdded'; 
  products: Product[] = [
  
      { image: 'assets/Hydratingserum.png', title: 'Hydrating Serum', description: 'with 11% Vitamin C', price: ' €18.00' , isLiked: false},
  ...
  goToProductPage(product: Product): void {
    if (product.title === 'Hydrating Serum') {
      this.router.navigate(['/hydratingserum']);
    } else if (product.title === 'Radiance Serum') {
      this.router.navigate(['/radianceserum']);
    } else if (product.title === 'Tanned complexion serum') {
      this.router.navigate(['/tanned']);
    } else if (product.title === 'Trio of serums') {
  ...
  navigateToProductType(event: any) {
    ...
    switch (selectedValue) {
      case 'scalp':
        this.navCtrl.navigateForward('/scalp');
        break;
      case 'shampoos':
        this.navCtrl.navigateForward('/shampoos');
        break;
      case 'bodyscrub':
        this.navCtrl.navigateForward('/bodyscrub');
        break;
      case 'showers':
        this.navCtrl.navigateForward('/showers');
        break;
      default:
        break;
    }
